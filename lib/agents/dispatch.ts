...
//   - Claude Code & Codex are CLI-first → build `claude "…"` / `codex "…"`
//     and copy to the clipboard.
//   - Cursor exposes `cursor://` → best-effort deep link + clipboard fallback.
//   - When the VS Code bridge is paired (see lib/session/relay), prompts go
//     through it instead of the clipboard.
...
export function buildAgentPrompt(input: PromptInput): string {
  const lines: string[] = []
  lines.push(`Working in: ${input.repo.name}`)
  lines.push('')
...
export interface DispatchOptions {
  /** Live editor bridge. Prefer this when paired. */
