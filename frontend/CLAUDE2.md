...
Decisions in this section *intentionally* override the spec sections referenced. They are user-approved and load-bearing — don't "fix" them back to the spec without checking in first.

| Override | Overrides | Date | Why |
|---|---|---|---|
| The map graphic adopts a **hand-drawn sketchy aesthetic** (roughjs strokes, pastel folder clouds, marker-feel labels, slight node tilt). | §9 *"Nothing on the canvas is decorative. No gradients or shadows unless they communicate something."* | 2026-05-12 | The product is meant to feel like a senior engineer drawing on a whiteboard for a new hire — that gesture is warm and imperfect, not minimalist. Encoding (color = language, size = LOC, border thickness = `commits_90d`) is preserved; only the *stroke style* is decorative. |
| The **VS Code extension + live browser↔editor bridge** ship in V1. | §10 *"VS Code extension. Possibly V2."* | 2026-05-12 | The bridge is core to the "ask in the workspace, edit in the IDE" loop; deferring it would split the agentic story across two releases. The dispatch fallbacks (clipboard, deep links) still work without the extension paired. |
