// Wire protocol shared between the workspace (browser) and the
// VS Code extension. Discriminated union so we can evolve safely;
// every consumer narrows on `type` and gets exhaustive checking.
...
export interface ReadyMsg extends MsgBase {
  type: "ready";
  /** What the editor side can do, e.g. ["prompt", "open-file", "selection"]. */
  capabilities: string[];
  /** Optional editor identification string ("vscode 1.95"). */
  clientName?: string;
}
...
export interface PromptMsg extends MsgBase {
  ...
  target: AgentTarget;
  text: string;
  /** Repo-relative paths the editor side can resolve as @-refs. */
  refs?: string[];
}
...
// Type guards — handy when handling the typed union.
...
/**
 * Parse and validate raw text from the wire. Returns null on any
 * malformed payload — callers should ignore those silently.
 */
...
/**
 * The handshake ack the editor sends as soon as a browser connects
